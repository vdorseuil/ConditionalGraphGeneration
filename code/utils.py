...
def preprocess_dataset(dataset, n_max_nodes, spectral_emb_dim,permute= False):
    ...
    if dataset == 'test':
        ...
        if permute: 
            augmented_list = ['adding_edges','adding_nodes','adding_triangles','changing_labels','changing_labels_bis']
        else: 
            augmented_list = ['adding_edges','adding_nodes','adding_triangles']
        for datatype in augmented_list:
            filename = './data/dataset_'+datatype+'.pt'
            graph_path = './data_augmented/'+datatype+'/graph'
            desc_path = './data_augmented/'+datatype+'/description'
            if os.path.isfile(filename):
                data_lst = torch.load(filename)
                print(f'Dataset {filename} loaded from file')
            else:
